...
fn get_sequence(req : &mut Request) -> IronResult<Response> {
	...
	request.sequence.id	= sequence.to_string();
...
fn get_next_sequence(req : &mut Request) -> IronResult<Response> {
	let conn = get_pg_connection!(req);
	let mut request = dal::ApplicationRequest::new();
	let auth_key = req.extensions.get::<Router>().unwrap().find("auth_key").unwrap_or("none");
	let sequence = req.extensions.get::<Router>().unwrap().find("sequence_key").unwrap_or("none");
	...
	match dal::update_sequence(&conn, &request) {
		Ok(sequence) => {
			let response_payload = try_or_400!(serde_json::to_string(&sequence));
			Ok(Response::with((status::Ok, response_payload)))
		},
...
fn main() {
	...
	router.get("/sequences/:auth_key/:sequence_key/current", get_sequence, "sequences/current");
    router.post("/sequences/:auth_key/:sequence_key/create", create_sequence, "sequences/create");
	router.get("/sequences/:auth_key/:sequence_key/next", get_next_sequence, "sequences/next");
